...
begin
  ...
  MessageDlg(Format('"%s" added', [GetTitleField.GetTValue.ToString]), TMsgDlgType.mtInformation, [TMsgDlgBtn.mbOK], 0);
  PopView;
  EditAddTitle.Text := '';
  MemoAddDescription.Text := '';
end;
...
begin
  ...
  if MessageDlg(Format('Delete "%s"?', [LTitle]), TMsgDlgType.mtConfirmation, mbOKCancel, 0) <> mrCancel then
  begin
    FBindSourceAdapter.Delete;
    MessageDlg(Format('"%s" deleted', [LTitle]), TMsgDlgType.mtInformation, [TMsgDlgBtn.mbOK], 0);
...
begin
  if (not FBindSourceAdapter.Modified) or (MessageDlg('Cancel changes?', TMsgDlgType.mtConfirmation, mbOKCancel, 0) <> mrCancel) then
  begin
