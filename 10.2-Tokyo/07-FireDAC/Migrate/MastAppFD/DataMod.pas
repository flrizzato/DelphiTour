...
uses
  ...
  FireDAC.Phys.Intf, FireDAC.Stan.Intf, FireDAC.Stan.Option, FireDAC.Stan.Error, 
...
type
  TMastData = class(TDataModule)
    ...
    procedure PartsUpdateError(ASender: TDataSet; AException: EFDException;
  ...
  private
    ...
    procedure SetDatabaseAlias(ConnectionDefName: string);
...
procedure TMastData.DatabaseBeforeConnect(Sender: TObject);
var
  fName: string;
  stl: TStringList;
begin
  ...
  try
    ...
    begin
      ...
      Database.Params.Values['DriverID'] := stl.ValueFromIndex[0];
      Database.Params.Values['Server'] := stl.ValueFromIndex[1];
      Database.Params.Values['Database'] := stl.ValueFromIndex[2];
      Database.Params.Values['User_Name'] := stl.ValueFromIndex[3];
      Database.Params.Values['Password'] := stl.ValueFromIndex[4];
    end;
  finally
    stl.Free;
  end;
...
procedure TMastData.SetDatabaseAlias(ConnectionDefName: string);
begin
  Screen.Cursor := crHourGlass;
  try
    Database.Close;
    Database.ConnectionDefName := ConnectionDefName;
    Database.Open;
  finally
 ...
 begin
   ...
   begin
     ...
     try
       ...
       Params.Values['DataBase'] := 'C:\DB_INTERBASE\MASTSQL.GDB';
...
procedure TMastData.PartsUpdateError(ASender: TDataSet;
...
begin
  if ARequest = arDelete then
    Key := ASender.Fields[0].OldValue else
    Key := ASender.Fields[0].NewValue;
  if MessageDlg(Format(UpdErrMsg, [AException.Message, ASender.Fields[0].DisplayLabel, Key]),
...
begin
  ...
  begin
    ...
    begin
      if Apply then
      begin
        Database.ApplyUpdates([DataSet as TFDRDBMSDataSet]);
