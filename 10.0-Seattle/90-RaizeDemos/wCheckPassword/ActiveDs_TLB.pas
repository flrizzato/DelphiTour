...
// File generated on 12/2/1999 1:09:14 PM from Type Library described below.
...
type
  ...
  ADS_UTC_TIME = TDateTime;
  ADS_LARGE_INTEGER = Int64;
  PADS_CASEIGNORE_LIST = ^ADS_CASEIGNORE_LIST;
  ADS_CASEIGNORE_LIST = packed record
    next: PADS_CASEIGNORE_LIST;
    str: PWideChar;
  end;
  PADS_OCTET_LIST = ^ADS_OCTET_LIST;
  ADS_OCTET_LIST = packed record
    next: PADS_OCTET_LIST;
    dwLength: DWORD;
    data: PBYTE;
  end;
  ADS_TYPENAME = packed record
    ObjectName: PWideChar;
    level: DWORD;
    Interval: DWORD;
  end;
  ...
  __MIDL___MIDL_itf_ads_0000_0017 = record
    case Integer of
      ...
      8: (UTCTime: ADS_UTC_TIME);
      9: (LargeInteger: ADS_LARGE_INTEGER);
      10: (ClassName: PWideChar);
      11: (ProviderSpecific: ADS_PROV_SPECIFIC);
      12: (pCaseIgnoreList: PADS_CASEIGNORE_LIST);
      13: (pOctetList: PADS_OCTET_LIST);
      14: (pPath: ^ADS_PATH);
      15: (pPostalAddress: ^ADS_POSTALADDRESS);
      16: (Timestamp: ADS_TIMESTAMP);
      17: (BackLink: ADS_BACKLINK);
      18: (pTypedName: ^ADS_TYPENAME);
      19: (Hold: ADS_HOLD);
      20: (pNetAddress: ^ADS_NETADDRESS);
      21: (pReplicaPointer: ^ADS_REPLICAPOINTER);
      22: (pFaxNumber: ^ADS_FAXNUMBER);
      23: (Email: ADS_EMAIL);
      24: (SecurityDescriptor: ADS_NT_SECURITY_DESCRIPTOR);
      25: (pDNWithBinary: ^ADS_DN_WITH_BINARY);
      26: (pDNWithString: ^ADS_DN_WITH_STRING);
  end;
  ...
  IDirectorySearch = interface(IUnknown)
    ...
    function SetSearchPreference(pSearchPrefs: pointer; // ads_searchpref_info;
      dwNumPrefs: LongWord): HResult; stdcall;

    function ExecuteSearch(pszSearchFilter: PWideChar;
      pAttributeNames: pointer;
      dwNumberAttributes: LongWord;
      out phSearchResult: ADS_SEARCH_HANDLE): HResult; stdcall;

    function AbandonSearch(hSearchHandle: ADS_SEARCH_HANDLE): HResult; stdcall;

    function GetFirstRow(hSearchHandle: ADS_SEARCH_HANDLE): HResult; stdcall;

    function GetNextRow(hSearchHandle: ADS_SEARCH_HANDLE): HResult; stdcall;

    function GetPreviousRow(hSearchHandle: ADS_SEARCH_HANDLE): HResult; stdcall;

    function GetNextColumnName(hSearchHandle: ADS_SEARCH_HANDLE;
      out ppszColumnName: PWideChar): HResult; stdcall;

    function GetColumn(hSearchHandle: ADS_SEARCH_HANDLE;
      szColumnName: PWideChar;
      const pSearchColumn: ADS_SEARCH_COLUMN): HResult; stdcall;

    function FreeColumn(const pSearchColumn: ADS_SEARCH_COLUMN): HResult; stdcall;

    function CloseSearchHandle(hSearchHandle: ADS_SEARCH_HANDLE): HResult; stdcall;
  end;
