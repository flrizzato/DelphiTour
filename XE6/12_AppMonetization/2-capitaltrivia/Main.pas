...
uses
  ...
  FMX.Advertising, FMX.InAppPurchase, System.Generics.Collections, FMX.Layouts;
...
uses Quiz, Option, Score, Answered;
...
begin
  ...
  begin
    OptionForm.ConsumeListBoxItem.Enabled := True;
    OptionForm.EuropeListBoxItem.Enabled := False;
    OptionForm.EuropeListBoxItem.OnClick := nil;
    if BecauseOfPurchase then
    begin
      OptionForm.DisableAdsListBoxItem.Text := 'Europe pack is now owned';
      OptionForm.ListBoxItem4.Visible  := True;
    end;
  end;
end;
...
begin
  ...
  begin
    OptionForm.ConsumeListBoxItem.Enabled := True;
    OptionForm.DisableAdsListBoxItem.Enabled := False;
    OptionForm.DisableAdsListBoxItem.OnClick := nil;
    if BecauseOfPurchase then
      OptionForm.DisableAdsListBoxItem.Text := 'In-app adverts are disabled';
  end;
end;

procedure TMainForm.EnableEuropePurchaseUI;
begin
  OptionForm.EuropeListBoxItem.Enabled := True;
end;

procedure TMainForm.EnablePurchaseUI;
begin
  OptionForm.DisableAdsListBoxItem.Enabled := True;
  OptionForm.RestoreAdsListBoxItem.Enabled := True;
end;
